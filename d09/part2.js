/**
 * starting IQR bench
 * avarage time part 2: 0.8661589914537351 ms
 */
...
export default function solve(input) {
    return input
        ...
        .map((line) => extrapolate_backwards(line))
        .reduce((total, line_sum) => total + line_sum, 0);
}

function extrapolate_backwards(line) {
    const pyramid = build_pyramid(line);
    const lines_lowest = pyramid.reduceRight((s, line) => line[0] - s, 0);
    return lines_lowest;
}
