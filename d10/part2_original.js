...
export default function solve(input) {
    ...
    const path = find_path(map);
    let [xs, ys, xg, yg] = [map[0].length - 1, map.length - 1, 0, 0];
    ...
    for (let y = ys; y <= yg; y++) {
        for (let x = xs; x <= xg; x++) {
            if (is_point_part_of_path({ x, y }, path)) continue;

            let x1 = x;
            let y1 = y;
            let intersections = 0;
            while (x1 >= xs && y1 >= ys) {
                if (++x1 > xg || ++y1 > yg) break;
                if (is_point_part_of_path({ x: x1, y: y1 }, path)) {
...
function is_point_part_of_path(point, path) {
    const point_of_path = path.find(({ x, y }) => x === point.x && y === point.y);
    return !!point_of_path;
}
